$(document).ready(function(){
	...
	function sethuku(data){
		...
		for(var i=0; i<data.question.length; i++){
    		$("#hukuall").append("<tr>"+"<td class='thcenter col-xs-1'><a href='"+ CONTEXT_PATH + "login/nenmondai?pagenumber=" + (i) + "' class='sessionstrclr2 no'>" + (i+1) + "</a></td>"+"<td class='col-xs-5'>" + data.question[i].ronten + "</td>"+"<td class='col-xs-2 thcenter'>" + data.question[i].yearname + "</td>"+"<td class='thcenter col-xs-1'><button class='btns btn btn-warning btn-xs' name='" + data.question[i].id + "'>解除</button></td></tr>");
